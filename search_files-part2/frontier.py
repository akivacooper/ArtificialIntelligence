...
def insert(frontier, s):
    #inserts state s to the frontier
    state=frontier[0]
    frontier[1]+=1
    state.append(s)     # inserts the new state as the last item
    i=len(state)-1      # i gets its value
    # move the item with smallest value to the root
    while i>0 and val(state[i])<val(state[(i-1)//2]): # while item i's value is smaller than the value of his father, swap!
