...
def hdistance1(s): #This will be the simple heuristic of the number of bricks not in place
    ...
    out_of_place = 0
    for count, next_int in enumerate(s[0]):
        if next_int != count:
            out_of_place += 1
...
def hdistance2(s): #This will be the Manhattan distance heuristic
    ...
    sideLength = math.sqrt(len(s[0]))
    MD=0

    for count, next_int in enumerate(s[0]):
        currentRow = count/sideLength
        currentColumn = count % sideLength
        idealRow = next_int/sideLength
        idealColumn = next_int % sideLength
        MD += (abs(currentRow - idealRow) + (abs(currentColumn - idealColumn)))
