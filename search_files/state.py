...
def create(boardWidth):
    s=list(range(boardWidth * boardWidth))      # s is the board itself. a vector that represent a matrix. s=[0,1,2....n^2-1]
    m="<>v^"                # m is "<>v^" - for every possible move (left, right, down, up)
    for i in range(boardWidth * boardWidth * boardWidth):  # makes n^3 random moves
